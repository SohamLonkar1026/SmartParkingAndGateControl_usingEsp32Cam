...
void setupCamera() {
  ...
  // Adjust frame size and quality for QR code scanning
  if(psramFound()){
    config.frame_size = FRAMESIZE_VGA; // 640x480 - better for QR codes
  ...
  // Optimize camera sensor settings for QR code scanning
  sensor_t *s = esp_camera_sensor_get();
  if (s) {
    // Increase sharpness and contrast for better QR detection
    s->set_brightness(s, 0);     // -2 to 2
    s->set_contrast(s, 1);       // -2 to 2 (increase for sharper edges)
    ...
    s->set_bpc(s, 1);            // 0 = disable , 1 = enable (black pixel correction)
    ...
    Serial.println("Camera settings optimized for QR scanning");
  }
}
